...
def generate_data_json(today_holdings, prev_holdings, data_date_str, aum_ntd=0, units=0):
    ...
    wrapper = {
        "meta": {
            "manager": MANAGER, "ytd": ytd_val, "etfPrice": etf_price, "priceChange": price_change,
...
def git_push():
    try:
        ...
        subprocess.run(["git", "commit", "-m", f"Auto-update 00985A holdings {datetime.now(timezone(timedelta(hours=8))).strftime('%Y-%m-%d %H:%M')}"], check=True)
